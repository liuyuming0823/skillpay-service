...
> **2026-09-18 文档修订**：更正了域名与 HTTPS 的定性——**资源地址必须是 HTTPS，否则收不到钱**
> （支付宝 CLI 会拒绝 HTTP 资源地址）。新增 Cloudflare 免备案隧道方案，以及上线后必须把
> HTTPS 地址同步回技能的两处位置。详见 `docs/接入教程.md` 第 5.7、5.8 节。
> 本次仅修订文档，`payloads/` 下的交付文件名与服务端代码均未变动。
